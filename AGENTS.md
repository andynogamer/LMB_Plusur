...
- **Markers are solved (2026-09-07).** `arcoreimg` measurement found four club
  ...
  The first Conspiradores silhouette, plus `el_aguila` and `pericos`, yield
  **zero keypoints**. A later Conspiradores wordmark scored 100 raw and is in
  the scan database. Águila and Pericos stay out until a file of theirs
  scores ≥ 75. Remaining work is print + device lock, then those two logos.
